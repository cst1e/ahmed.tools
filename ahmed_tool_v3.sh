...
# ==================================================
# 🎨 أداة أحمد لأدوات اختبار الاختراق (v3 - واجهة نصية محسنة)
...
echo -e "   ${S_WARN} ${C_BOLD}${C_YELLOW}تنبيه هام وإخلاء مسؤولية${C_RESET} ${S_WARN}"
echo -e "${C_BOLD}${C_YELLOW}==================================================${C_RESET}"
echo -e "${C_DIM}هذا السكربت \"أحمد\" مصمم للأغراض ${C_BOLD}التعليمية والبحثية فقط${C_DIM} في مجال الأمن السيبراني واختبار الاختراق الأخلاقي.${C_RESET}"
echo -e "${C_DIM}يُحظر تمامًا استخدام هذه الأداة في أي أنشطة ${C_BOLD}غير قانونية أو غير مصرح بها${C_DIM}.${C_RESET}"
echo -e "${C_DIM}المطور (\"أحمد\") والمساهمون لا يتحملون أي مسؤولية عن أي استخدام غير قانوني أو ضار لهذه الأداة.${C_RESET}"
echo -e "${C_DIM}أنت وحدك المسؤول عن أفعالك وعن التأكد من امتثالك لجميع القوانين واللوائح المعمول بها قبل استخدام أي من الأدوات المدرجة.${C_RESET}"
...
show_main_menu() {
    clear
    # ASCII Art Header (Simple Example)
    ...
    echo "    ██╔══██║██║╚██╔╝██║██║   ██║╚════██║╚════██║"
    echo "    ██║  ██║██║ ╚═╝ ██║╚██████╔╝███████║███████║"
    echo "    ╚═╝  ╚═╝╚═╝     ╚═╝ ╚═════╝ ╚══════╝╚══════╝ ${C_MAGENTA}v3${C_BLUE}"
    echo -e "          ${C_DIM}أداة أحمد لأدوات اختبار الاختراق${C_RESET}"
    print_separator
    ...
    print_separator
    read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر فئة: ${C_RESET}" main_choice
}
...
menu_info_gathering() {
    while true; do
        clear
        print_title "1. جمع المعلومات"
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}فحص الشبكة ${C_DIM}(Nmap)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}2)${C_RESET} ${C_WHITE}جمع معلومات OSINT ${C_DIM}(theHarvester)${C_RESET}"
        echo -e "   ${S_RETURN} ${C_YELLOW}3)${C_RESET} ${C_WHITE}رجوع للقائمة الرئيسية${C_RESET}"
        print_separator
        read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر أداة: ${C_RESET}" sub_choice
        case $sub_choice in
            1) read -p $"${S_PROMPT} ${C_WHITE}أدخل IP أو نطاق الهدف لـ Nmap: ${C_RESET}" target; 
               if [ -n "$target" ]; then run_command_pretty "Nmap Scan" "nmap -sV -A $target"; fi ;; 
            2) read -p $"${S_PROMPT} ${C_WHITE}أدخل اسم النطاق لـ theHarvester: ${C_RESET}" domain; 
...
menu_vuln_analysis() {
    while true; do
        clear
        print_title "2. تحليل الثغرات"
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}فحص ثغرات الويب ${C_DIM}(Nikto)${C_RESET}"
        echo -e "   ${S_RETURN} ${C_YELLOW}2)${C_RESET} ${C_WHITE}رجوع للقائمة الرئيسية${C_RESET}"
        print_separator
        read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر أداة: ${C_RESET}" sub_choice
        case $sub_choice in
            1) read -p $"${S_PROMPT} ${C_WHITE}أدخل رابط الموقع (e.g., http://example.com): ${C_RESET}" url; 
...
menu_web_app_analysis() {
    while true; do
        clear
        print_title "3. تحليل تطبيقات الويب"
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}تشغيل Burp Suite${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}2)${C_RESET} ${C_WHITE}فحص حقن SQL ${C_DIM}(SQLMap)${C_RESET}"
        echo -e "   ${S_RETURN} ${C_YELLOW}3)${C_RESET} ${C_WHITE}رجوع للقائمة الرئيسية${C_RESET}"
        print_separator
        read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر أداة: ${C_RESET}" sub_choice
        case $sub_choice in
            1) echo -e "${S_INFO} ${C_YELLOW}تشغيل Burp Suite في الخلفية...${C_RESET}"; burpsuite & pause ;; 
            2) read -p $"${S_PROMPT} ${C_WHITE}أدخل رابط URL للفحص باستخدام SQLMap: ${C_RESET}" sql_url; 
...
menu_password_attacks() {
    while true; do
        clear
        print_title "4. هجمات كلمة المرور"
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}كسر كلمات المرور ${C_DIM}(John the Ripper)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}2)${C_RESET} ${C_WHITE}تخمين كلمات المرور أونلاين ${C_DIM}(Hydra)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}3)${C_RESET} ${C_WHITE}إنشاء قوائم كلمات ${C_DIM}(Crunch)${C_RESET}"
        echo -e "   ${S_RETURN} ${C_YELLOW}4)${C_RESET} ${C_WHITE}رجوع للقائمة الرئيسية${C_RESET}"
        print_separator
        read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر أداة: ${C_RESET}" sub_choice
        case $sub_choice in
            1) read -p $"${S_PROMPT} ${C_WHITE}أدخل مسار ملف الهاش لـ John: ${C_RESET}" hash_file; 
            ...
            2) echo -e "${S_INFO} ${C_DIM}مثال لاستخدام Hydra: hydra -L users.txt -P pass.txt ftp://192.168.1.1${C_RESET}"; 
               read -p $"${S_PROMPT} ${C_WHITE}أدخل أمر Hydra كاملاً: ${C_RESET}" hydra_cmd; 
               if [ -n "$hydra_cmd" ]; then run_command_pretty "Hydra Attack" "$hydra_cmd"; fi ;; 
            3) read -p $"${S_PROMPT} ${C_WHITE}أدخل الحد الأدنى لطول الكلمة: ${C_RESET}" min; 
...
menu_exploitation_tools() {
    while true; do
        clear
        print_title "5. أدوات الاستغلال"
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}إنشاء بايلود ${C_DIM}(MSFvenom)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}2)${C_RESET} ${C_WHITE}تشغيل Metasploit Framework ${C_DIM}(msfconsole)${C_RESET}"
        echo -e "   ${S_RETURN} ${C_YELLOW}3)${C_RESET} ${C_WHITE}رجوع للقائمة الرئيسية${C_RESET}"
        print_separator
        read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر أداة: ${C_RESET}" sub_choice
        case $sub_choice in
            1) 
                read -p $"${S_PROMPT} ${C_WHITE}أدخل نوع البايلود (e.g., windows/meterpreter/reverse_tcp): ${C_RESET}" payload
...
menu_wireless_attacks() {
    ...
    while true; do
        clear
        print_title "6. هجمات الشبكات اللاسلكية"
        ...
        print_separator
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}اختيار واجهة وتفعيل وضع المراقبة ${C_DIM}(Airmon-ng)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}2)${C_RESET} ${C_WHITE}فحص الشبكات المجاورة ${C_DIM}(Airodump-ng)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}3)${C_RESET} ${C_WHITE}التقاط Handshake ${C_DIM}(Airodump-ng)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}4)${C_RESET} ${C_WHITE}هجوم Deauthentication ${C_DIM}(Aireplay-ng)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}5)${C_RESET} ${C_WHITE}كسر كلمة مرور WPA/WPA2 ${C_DIM}(Aircrack-ng)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}6)${C_RESET} ${C_WHITE}فحص شبكات WPS ${C_DIM}(Wash)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}7)${C_RESET} ${C_WHITE}هجوم WPS PIN ${C_DIM}(Reaver)${C_RESET}"
        ...
        print_separator
        read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر أداة أو إجراء: ${C_RESET}" sub_choice
        ...
        case $sub_choice in
            ...
            5) 
                ...
                echo -e "${S_INFO} ${C_YELLOW}بدء محاولة كسر كلمة المرور باستخدام Aircrack-ng...${C_RESET}"
                run_command_pretty "Aircrack-ng Attack" "sudo aircrack-ng $capture_file_path -w $wordlist_path"
                ;; 
            6) 
                if [ -z "$current_monitor_interface" ]; then echo -e "${S_WARN} ${C_YELLOW}يرجى تفعيل وضع المراقبة أولاً (الخيار 1).${C_RESET}"; pause; continue; fi
                echo -e "${S_INFO} ${C_CYAN}بدء فحص شبكات WPS المجاورة باستخدام Wash... اضغط Ctrl+C للإيقاف.${C_RESET}"
                run_command_pretty "Wash WPS Scan" "sudo wash -i $current_monitor_interface"
                ;; 
            7) 
                if [ -z "$current_monitor_interface" ]; then echo -e "${S_WARN} ${C_YELLOW}يرجى تفعيل وضع المراقبة أولاً (الخيار 1).${C_RESET}"; pause; continue; fi
                read -p $"${S_PROMPT} ${C_WHITE}أدخل BSSID للشبكة المستهدفة (يجب أن تدعم WPS): ${C_RESET}" target_bssid
                if [ -n "$target_bssid" ]; then
                    echo -e "${S_INFO} ${C_YELLOW}بدء هجوم WPS PIN باستخدام Reaver على ${C_BOLD}$target_bssid${C_RESET}${C_YELLOW}... قد يستغرق وقتاً طويلاً. اضغط Ctrl+C للإيقاف.${C_RESET}"
...
menu_sniffing_spoofing() {
    while true; do
        clear
        print_title "7. التنصت والخداع"
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}تشغيل Wireshark ${C_DIM}(تحليل الحزم)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}2)${C_RESET} ${C_WHITE}تشغيل Ettercap ${C_DIM}(رسومي - MITM)${C_RESET}"
        echo -e "   ${S_RETURN} ${C_YELLOW}3)${C_RESET} ${C_WHITE}رجوع للقائمة الرئيسية${C_RESET}"
        print_separator
        read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر أداة: ${C_RESET}" sub_choice
        case $sub_choice in
            1) echo -e "${S_INFO} ${C_YELLOW}تشغيل Wireshark...${C_RESET}"; sudo wireshark & pause ;; 
            2) echo -e "${S_INFO} ${C_YELLOW}تشغيل Ettercap بواجهة رسومية...${C_RESET}"; sudo ettercap -G & pause ;; 
            3) break ;; # رجوع
            *) echo -e "${S_ERR} ${C_RED}خيار غير صالح${C_RESET}"; sleep 1 ;; 
        esac
    done
