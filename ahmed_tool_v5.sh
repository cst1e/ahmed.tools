...
# ==================================================
# 🎨 أداة أحمد لأدوات اختبار الاختراق (v5 - واجهة نصية محسنة)
...
# استخدام $'\[...m' لضمان تفسير الـ escape sequence بشكل صحيح
C_RESET=$'\e[0m'
C_BOLD=$'\e[1m'
C_DIM=$'\e[2m'
C_RED=$'\e[31m'
...
echo -e "${C_BOLD}${BG_YELLOW}${C_RED}==================================================${C_RESET}"
echo -e "${C_DIM}${C_WHITE}هذا السكربت \"أحمد\" مصمم للأغراض ${C_BOLD}${C_YELLOW}التعليمية والبحثية فقط${C_RESET}${C_DIM}${C_WHITE} في مجال الأمن السيبراني.${C_RESET}"
echo -e "${C_DIM}${C_WHITE}يُحظر تمامًا استخدامه في أي أنشطة ${C_BOLD}${C_RED}غير قانونية أو غير مصرح بها${C_RESET}${C_DIM}${C_WHITE}.${C_RESET}"
echo -e "${C_DIM}${C_WHITE}المطور والمساهمون لا يتحملون أي مسؤولية عن أي استخدام غير قانوني أو ضار.${C_RESET}"
echo -e "${C_DIM}${C_WHITE}أنت وحدك المسؤول عن أفعالك وعن التأكد من امتثالك للقوانين المعمول بها.${C_RESET}"
echo -e "${C_BOLD}${C_RED}باستخدام هذا السكربت، فإنك توافق على هذه الشروط وتتحمل المسؤولية الكاملة.${C_RESET}"
echo -e "${C_BOLD}${BG_YELLOW}${C_RED}==================================================${C_RESET}"
...
print_separator() {
    # استخدام لون مختلف للفاصل
    echo -e "${C_DIM}${C_MAGENTA}==================================================${C_RESET}"
}

# دالة لطباعة عنوان قسم
print_title() {
    print_separator
    # Alternating colors for titles (Blue/Magenta)
    local color=${C_MAGENTA}
    # استخدام طريقة أبسط للتبديل
    if (( RANDOM % 2 == 0 )); then
       color=${C_BLUE}
    fi 
    echo -e "   ${C_BOLD}${color}$1${C_RESET}"
    print_separator
}
...
show_main_menu() {
    clear
    # ASCII Art Header (AHMED - Corrected)
    ...
    echo '   ██║  ██║██║  ██║██║ ╚═╝ ██║███████╗██████╔╝'
    echo '   ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝╚═════╝ '
    echo -e "${C_RESET}" # Reset color after logo
    echo -e "          ${C_DIM}أداة أحمد لأدوات اختبار الاختراق ${C_MAGENTA}v5${C_RESET}"
    print_separator
...
run_command_pretty() {
    ...
    local cmd="$2"
    clear
    # استخدام لون مختلف لعنوان التشغيل
    print_title "${S_TOOL} ${C_CYAN}جارٍ تشغيل: $title${C_RESET}"
...
menu_info_gathering() {
    while true; do
        ...
        # استخدام ألوان متنوعة لأسماء الأدوات
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}فحص الشبكة ${C_BOLD}${C_MAGENTA}(Nmap)${C_RESET}"
        ...
        case $sub_choice in
            1) read -p $"${S_PROMPT} ${C_WHITE}أدخل IP أو نطاق الهدف لـ ${C_BOLD}${C_MAGENTA}Nmap${C_RESET}: ${C_RESET}" target; 
...
menu_vuln_analysis() {
    while true; do
        ...
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}فحص ثغرات الويب ${C_BOLD}${C_MAGENTA}(Nikto)${C_RESET}"
        ...
        case $sub_choice in
            1) read -p $"${S_PROMPT} ${C_WHITE}أدخل رابط الموقع (e.g., http://example.com) لـ ${C_BOLD}${C_MAGENTA}Nikto${C_RESET}: ${C_RESET}" url; 
...
menu_password_attacks() {
    while true; do
        ...
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}كسر كلمات المرور ${C_BOLD}${C_MAGENTA}(John the Ripper)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}2)${C_RESET} ${C_WHITE}تخمين كلمات المرور أونلاين ${C_BOLD}${C_BLUE}(Hydra)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}3)${C_RESET} ${C_WHITE}إنشاء قوائم كلمات ${C_BOLD}${C_MAGENTA}(Crunch)${C_RESET}"
        ...
        case $sub_choice in
            1) read -p $"${S_PROMPT} ${C_WHITE}أدخل مسار ملف الهاش لـ ${C_BOLD}${C_MAGENTA}John${C_RESET}: ${C_RESET}" hash_file; 
               if [ -n "$hash_file" ]; then run_command_pretty "John the Ripper" "john \"$hash_file\""; fi ;; # Added quotes for path
            ...
            3) read -p $"${S_PROMPT} ${C_WHITE}أدخل الحد الأدنى لطول الكلمة لـ ${C_BOLD}${C_MAGENTA}Crunch${C_RESET}: ${C_RESET}" min; 
...
menu_wireless_attacks() {
    ...
    while true; do
        ...
        print_separator
        # استخدام ألوان متنوعة للأدوات اللاسلكية
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}اختيار واجهة وتفعيل وضع المراقبة ${C_BOLD}${C_MAGENTA}(Airmon-ng)${C_RESET}"
        ...
        echo -e "   ${S_TOOL} ${C_CYAN}4)${C_RESET} ${C_WHITE}هجوم Deauthentication ${C_BOLD}${C_MAGENTA}(Aireplay-ng)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}5)${C_RESET} ${C_WHITE}كسر كلمة مرور WPA/WPA2 ${C_BOLD}${C_BLUE}(Aircrack-ng)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}6)${C_RESET} ${C_WHITE}فحص شبكات WPS ${C_BOLD}${C_MAGENTA}(Wash)${C_RESET}"
        ...
        case $sub_choice in
            ...
            6) 
                if [ -z "$current_monitor_interface" ]; then echo -e "${S_WARN} ${C_YELLOW}يرجى تفعيل وضع المراقبة أولاً (الخيار 1).${C_RESET}"; pause; continue; fi
                echo -e "${S_INFO} ${C_CYAN}بدء فحص شبكات WPS المجاورة باستخدام ${C_BOLD}${C_MAGENTA}Wash${C_RESET}${C_CYAN}... اضغط Ctrl+C للإيقاف.${C_RESET}"
...
menu_sniffing_spoofing() {
    while true; do
        ...
        echo -e "   ${S_TOOL} ${C_CYAN}1)${C_RESET} ${C_WHITE}تشغيل ${C_BOLD}${C_MAGENTA}Wireshark${C_RESET} ${C_DIM}(تحليل الحزم)${C_RESET}"
        echo -e "   ${S_TOOL} ${C_CYAN}2)${C_RESET} ${C_WHITE}تشغيل ${C_BOLD}${C_BLUE}(Ettercap)${C_RESET} ${C_DIM}(رسومي - MITM)${C_RESET}"
        echo -e "   ${S_RETURN} ${C_YELLOW}3)${C_RESET} ${C_WHITE}رجوع للقائمة الرئيسية${C_RESET}"
        print_separator
        read -p $"${S_PROMPT} ${C_BOLD}${C_WHITE}اختر أداة [1-3]: ${C_RESET}" sub_choice
        case $sub_choice in
            1) echo -e "${S_INFO} ${C_YELLOW}تشغيل ${C_BOLD}${C_MAGENTA}Wireshark${C_RESET}${C_YELLOW}...${C_RESET}"; sudo wireshark & pause ;; 
